...
export default function ProductDetailsPage() {
  ...
  const [reviews, setReviews] = useState([
    { author: "John D.", content: "Incredible sound quality!", rating: 5, date: "2026-05-12" },
    { author: "Sarah M.", content: "Best ANC ever!", rating: 5, date: "2026-06-02" },
    { author: "Alex K.", content: "Battery lasts days!", rating: 5, date: "2026-06-14" },
  ]);
  ...
  const handleBuyNow = () => {
    alert(`Proceeding straight to secure checkout with 1x ${product.title} ($${product.price.toFixed(2)})!`);
  };
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-[#0F172A] antialiased">
      ...
      <main className="max-w-6xl mx-auto px-4 py-8 md:py-12 space-y-16">
              ...
              {[
                ...
                <button
                  ...
                  className={`flex flex-col items-center p-2 bg-[#F1F5F9] rounded-xl border-2 transition-all ${
                    activeImage === thumb.id ? "border-[#0F172A] bg-white shadow-xs" : "border-transparent opacity-70 hover:opacity-100"
                  }`}
                >
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-3 pt-2">
              ...
              <Button 
                ...
                className="w-full bg-[#334155] hover:bg-[#1E293B] text-white font-bold tracking-wide rounded-xl py-6.5 text-sm transition-colors shadow-xs"
              >
                BUY NOW
              </Button>
            ...
            <div className="grid grid-cols-2 gap-3 bg-white border border-zinc-200/60 rounded-xl p-4 text-xs font-bold text-zinc-600 shadow-2xs">
              <div className="flex items-center gap-2">🛡️ <span>Secure Checkout</span></div>
              <div className="flex items-center gap-2">📦 <span>Fast Shipping</span></div>
              <div className="flex items-center gap-2">🔄 <span>30-Day Money Back</span></div>
              <div className="flex items-center gap-2">✅ <span>2-Year Warranty</span></div>
            </div>
          </div>
        </div>
        ...
        <div className="border-t border-zinc-200 pt-12 space-y-8">
          <h2 className="text-xl font-black tracking-tight text-[#0F172A] uppercase">Customer Reviews</h2>
          
          {/* Display Existing Review Cards Grid */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {reviews.map((review, i) => (
              <div key={i} className="p-5 bg-white border border-zinc-200/60 rounded-xl shadow-2xs flex flex-col justify-between">
                <div>
                  <div className="flex text-amber-500 text-xs mb-2">
                    ...
                    {[...Array(5 - review.rating)].map((_, rIdx) => (
                      <span key={rIdx} className="text-zinc-200">★</span>
                    ))}
                  </div>
                  <p className="text-sm font-bold text-[#0F172A]">"{review.content}"</p>
                </div>
                <div className="flex items-center justify-between mt-4 pt-4 border-t border-zinc-100">
                  <div className="flex items-center gap-2">
                    <div className="w-6 h-6 rounded-full bg-[#E2E8F0] flex items-center justify-center text-[10px] font-bold">👤</div>
                    <span className="text-xs font-bold text-zinc-500">{review.author}</span>
                  </div>
          ...
          <div className="bg-white border border-zinc-200/80 rounded-2xl p-6 max-w-2xl shadow-2xs">
            <h3 className="text-sm font-black uppercase tracking-wider text-zinc-400 mb-4">Share Your Experience</h3>
            <form onSubmit={handleReviewSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-zinc-600 block">Rating</label>
                  <select 
                    value={newRating} 
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-zinc-600 block">Comment Review</label>
                <textarea 
                  rows="3"
                  value={newContent}
        ...
        <div className="border-t border-zinc-200 pt-12 space-y-4">
          <h2 className="text-xl font-black tracking-tight text-[#0F172A] uppercase mb-2">Frequently Asked Questions</h2>
          
          {[
            { id: "battery", q: "Battery life?", a: "Up to 40 hours of playback time with active noise cancelling turned on." },
            { id: "compatibility", q: "Compatibility?", a: "Fully compatible via Bluetooth 5.2 with iOS, Android, macOS, and Windows operating systems." },
            { id: "noise", q: "Noise cancellation?", a: "Features ultra-dense advanced hybrid active noise isolation capturing external waves perfectly." },
          ].map((faq) => (
            <div key={faq.id} className="border-b border-zinc-200 bg-white rounded-xl shadow-2xs border px-4 overflow-hidden">
              <button
                onClick={() => toggleFaq(faq.id)}
                className="w-full py-4 flex items-center justify-between text-left text-sm font-bold text-[#0F172A] focus:outline-hidden"
              >
                <span>{faq.q}</span>
                <span className="text-xs text-zinc-400">{openFaq[faq.id] ? "▲" : "▼"}</span>
              </button>
              {openFaq[faq.id] && (
        ...
        <div className="border-t border-zinc-200 pt-12 space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-black tracking-tight text-[#0F172A] uppercase">Related Products</h2>
            <div className="flex gap-1">
              <button className="w-8 h-8 rounded-full border border-zinc-200 flex items-center justify-center text-xs hover:bg-white transition-all">◀</button>
              <button className="w-8 h-8 rounded-full border border-zinc-200 flex items-center justify-center text-xs hover:bg-white transition-all">▶</button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { title: "Earbuds", price: 299.00, original: 399.00 },
              { title: "Charger", price: 299.00, original: 399.00 },
              { title: "Case", price: 299.00, original: 399.00 },
            ].map((item, i) => (
              <div key={i} className="group cursor-pointer">
                <div className="w-full aspect-4/3 bg-[#F1F5F9] rounded-xl flex items-center justify-center p-6 border border-zinc-200/20 overflow-hidden mb-3 group-hover:shadow-xs transition-all">
                  <img src="https://images.unsplash.com/photo-1546435770-a3e426bf472b?q=80&w=300&auto=format&fit=crop" className="w-3/5 h-3/5 object-contain mix-blend-multiply group-hover:scale-105 transition-transform" />
                </div>
                <h4 className="text-sm font-black text-[#0F172A]">{item.title}</h4>
                <div className="flex text-[10px] text-amber-500 my-0.5">★★★★★ <span className="text-zinc-400 font-medium ml-1">(45)</span></div>
                <div className="flex items-center gap-2">
                  <span className="text-sm font-black text-[#FF7A00]">${item.price.toFixed(2)}</span>
                  <span className="text-xs text-zinc-400 line-through">${item.original.toFixed(2)}</span>
                </div>
              </div>
            ))}
          </div>
